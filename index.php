...
<head>
    ...
    <title>Dashboard Jadwal OSCE</title>
...
.sidebar-header h3 { margin: 0; font-size: 1.2rem; color: var(--header-bg); }

.station-count {
    background-color: var(--accent-color); color: white; padding: 4px 10px;
...
.schedule-list li.active { background-color: #fffbe6; font-weight: 700; }

.schedule-list li.played {
    background-color: #f8f9f9; color: #b0b0b0; opacity: 0.7;
}
.schedule-list li.played .item-text { text-decoration: line-through; }
.schedule-list li.played .item-status-icon { color: #b0b0b0; }

.item-status-icon { font-size: 1rem; color: #7f8c8d; width: 20px; text-align: center; }
.item-status-icon.played-icon { color: #27ae60; }
.item-time { font-weight: 700; margin-right: 10px; }
...
.main-header h1 { margin: 0; font-size: 1.8rem; color: var(--header-bg); }

.clock {
    font-family: 'Orbitron', sans-serif; font-size: 2rem;
    background-color: var(--primary-bg); padding: 5px 15px; border-radius: 8px;
}
...
.countdown-timer {
    ...
    font-weight: 700; color: var(--accent-color); line-height: 1;
}
.next-schedule-text { font-size: 1.5rem; margin-top: 15px; color: var(--text-dark); }
.status-panel { text-align: center; margin-bottom: 20px; }

.status-badge {
    padding: 8px 15px; border-radius: 20px; color: white; font-weight: 700;
}
.status-idle { background-color: #7f8c8d; }
.status-running { background-color: #27ae60; }
.status-playing { background-color: #e67e22; }

.main-footer {
    text-align: center; padding-top: 20px;
    border-top: 1px solid var(--border-color); font-size: 0.9rem; color: #7f8c8d;
}
...
.audio-playing-text {
    font-size: 1.5rem; font-weight: 500; margin-top: 20px;
...
.startup-overlay {
    ...
    background-color: rgba(44, 62, 80, 0.95); z-index: 1000;
    display: flex; justify-content: center; align-items: center;
    color: white; text-align: center; transition: opacity 0.5s ease, visibility 0.5s;
}
.startup-box { padding: 40px; background-color: rgba(0, 0, 0, 0.2); border-radius: 15px; }
.startup-box h1 { font-size: 2.5rem; margin: 0; }
.startup-box p { font-size: 1.2rem; opacity: 0.9; }
.start-button {
    background-color: #27ae60; color: white; border: none; padding: 15px 40px;
    font-size: 1.5rem; font-weight: 700; border-radius: 10px; cursor: pointer;
    margin: 20px 0; transition: background-color 0.3s, transform 0.2s;
}
.start-button:hover { background-color: #2ecc71; transform: scale(1.05); }
.small-text { font-size: 0.9rem; opacity: 0.7; }
...
const stationConfiguration = [
    { name: "STASIUN 1: Anamnesis", durationInMinutes: 10, readDurationInMinutes: 1, warningTimeInMinutes: 3 },
    { name: "STASIUN 2: Pemeriksaan Fisik", durationInMinutes: 12, readDurationInMinutes: 1, warningTimeInMinutes: 3 },
    { name: "STASIUN 3: Tatalaksana", durationInMinutes: 10, readDurationInMinutes: 2, warningTimeInMinutes: 2 },
    { name: "STASIUN 4: Edukasi Pasien", durationInMinutes: 8, readDurationInMinutes: 1, warningTimeInMinutes: 3 },
    { name: "STASIUN 5: Keterampilan Prosedural", durationInMinutes: 15, readDurationInMinutes: 2, warningTimeInMinutes: 5 },
...
async function playSequence(index) {
    ...
    try {
        await new Promise((res, rej) => { ringtoneAwal.onended = res; ringtoneAwal.onerror = rej; ringtoneAwal.play().catch(rej); });
        await new Promise((res, rej) => {
            synth.cancel();
            const utterance = new SpeechSynthesisUtterance(item.audioText);
            utterance.lang = 'id-ID';
            utterance.onend = res; utterance.onerror = rej;
            synth.speak(utterance);
        });
        await new Promise((res, rej) => { ringtoneAkhir.onended = res; ringtoneAkhir.onerror = rej; ringtoneAkhir.play().catch(rej); });
    } catch (error) {
